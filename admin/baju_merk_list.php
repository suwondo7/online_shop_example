...
<table class="table-list" width="50%">
	...
	$qSQL = "SELECT * FROM _baju_merk WHERE (nama LIKE '%$keyword%') ORDER BY id_baju_merk ASC";
	$hqSQL = $db->sql($qSQL);
	...
	if($totalData=='0'){
		...
		while($hasil = $db->fetch_assoc($hqSQL)){
			...
			echo "<td align='center' width='5%'><a href='#update' onClick=\"javascript: sendRequest('baju_merk.php', 'id=$hasil[id_baju_merk]&add=false', 'content', 'div', '../images/loader.gif'); \"><img src='../images/edit.gif' border='0' title='Edit Data' /></a></td>";
			echo "<td align='center' width='5%'><a href='#del' onClick=\"javascript: if(confirm('Yakin dihapus ta?')) sendRequest('baju_merk_proses.php', 'proc=delete&id=$hasil[id_baju_merk]', 'list', 'div', '../images/loader.gif');\"><img src='../images/delete.gif' border='0' title='Hapus Data' /></a></td>";
			echo "</tr>";
...
<table width="50%" class="paging">
	<tr>
		...
		<td align="right">
			Halaman&nbsp;:&nbsp;
			<select name="f_paging" onchange="javascript: sendRequest('baju_merk_list.php','ajax=true&start='+parseInt(this.value)*<?php echo $limit; ?>, 'list', 'div');">
			<?php
