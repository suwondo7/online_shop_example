...
<h2>Produk Baru</h2>
<?php
$qData = "SELECT id_baju, foto, nama, keterangan, harga, id_baju_kategori FROM _baju ORDER BY tanggal DESC LIMIT 0, 4";
$hqData = $db->sql($qData);
...
if($rqData == '0'){
	...
	while($rqData = $db->fetch_assoc($hqData)){
		...
		list($bajuKat) 		= $db->result_assoc("SELECT nama FROM _baju_kategori WHERE id_baju_kategori = '$rqData[id_baju]'");
